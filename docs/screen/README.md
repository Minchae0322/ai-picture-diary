...
> 시안: [AI 감정 날씨일기 · Web App Sketch](https://www.figma.com/design/zbYEQIRk1ttPOFzojh2hcL/AI-%25EA%25B0%2590%25EC%25A0%2595-%25EB%2582%25A0%25EC%2594%25A8%25EC%259D%25BC%25EA%25B8%25B0---Web-App-Sketch?node-id=2-7) (페이지 `01 · Mobile Screens`, 390x844) · 갱신: 2026-09-22 (10화면 전부 구현)
> 작성 규칙: `.claude/skills/ai-picture-diary/screens/README.md`
...
- 07·09는 10 마이페이지에서 진입(설정 목록의 "뱃지" / "테마 변경"). 시안에 07 진입 동선이 없어 행을 하나 더했다.
- 과거 날짜의 04는 스택 라우트 `app/diary/[id].tsx`. 05 캘린더와 02 최근 기록에서 들어간다.

### 라우트 -> 화면

| 라우트 | 화면 |
|---|---|
| `app/onboarding.tsx` | 01 |
| `app/(tabs)/index.tsx` | 02 / 03 / 04 (오늘 기록 상태로 분기) |
| `app/diary/[id].tsx` | 04 (과거 날짜) |
| `app/(tabs)/calendar.tsx` | 05 |
| `app/(tabs)/graph.tsx` | 06 |
| `app/badges.tsx` | 07 |
| `app/(tabs)/community.tsx` | 08 |
| `app/store.tsx` | 09 |
| `app/(tabs)/my.tsx` | 10 |
...
## 시안에 없어 전역으로 미정인 것
- 로그인·회원가입 화면, 계정 삭제 (01의 "이미 계정이 있어요" 목적지) -> `spring-auth`
- 커뮤니티 댓글·게시글 상세 화면 (08 댓글 아이콘 목적지)
- 구독 결제 화면과 복원 동선 (09 "구독" 목적지)
- 설정 하위 화면 (10의 리마인더 / AI 그림 스타일 / 데이터 내보내기 / 계정·로그아웃)

> 과거 날짜 상세는 `app/diary/[id].tsx`로 확정됐다(04와 같은 레이아웃, 공유 버튼 없음).
